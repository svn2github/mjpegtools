/*
   ...
   $Id: zr36050.c,v 1.1.2.1 2002-08-01 11:05:44 rbultje Exp $
...
static __u8 zr36050_read(struct zr36050 *ptr, __u16 reg)
{       __u8 value = 0;
...
static void zr36050_write(struct zr36050 *ptr, __u16 reg, __u8 value)
{
...
static __u8 zr36050_read_status1(struct zr36050 *ptr)
{
...
static __u16 zr36050_read_scalefactor(struct zr36050 *ptr)
{
...
static int zr36050_pushit(struct zr36050 *ptr, __u16 startreg, __u16 len,
                          const char data[])
{
        int i=0;
...
static int zr36050_set_dri(struct zr36050 *ptr)
{
        ...
        dri_data[4]=ptr->dri>>8; dri_data[5]=ptr->dri*0xff;
...
static void zr36050_init(struct zr36050 *ptr)
{
        ...
        if (ptr->mode == CODEC_DO_COMPRESSION) {
                ...
                zr36050_write(ptr, ZR050_MODE, ZR050_MO_COMP | ZR050_MO_TLM );
                ...
                zr36050_write(ptr, ZR050_MODE, ZR050_MO_COMP | ZR050_MO_PASS2 | 
                                              (ptr->bitrate_ctrl ? ZR050_MO_BRC : 0) );

                /* this headers seem to deliver "valid AVI" jpeg frames */
                zr36050_write(ptr, ZR050_MARKERS_EN, ZR050_ME_APP |
                                                     ZR050_ME_DQT |
                                                     ZR050_ME_DHT |
                                                     ZR050_ME_DHTI );
        } else {
                ...
                /* 050 communicates with 055 in master mode */
                zr36050_write(ptr, ZR050_HARDWARE, ZR050_HW_MSTR | ZR050_HW_CFIS_2_CLK );
                ...
                DEBUG2("%s: write DHT\n",ptr->name);
                zr36050_pushit(ptr,ZR050_DHT_IDX,
                                    sizeof(zr36050_dht),zr36050_dht);
...
static void zr36050_inc_use(void)
{
        MOD_INC_USE_COUNT;
}

static void zr36050_dec_use(void)
{
        MOD_DEC_USE_COUNT;
}
...
int zr36050_unset(struct videocodec *codec)
...
int zr36050_setup(struct videocodec *codec)
{
        struct zr36050 *ptr;
        int res;

        DEBUG1("zr36050: initializing MJPEG subsystem #%d.\n",
               zr36050_codecs);

        if (zr36050_codecs==MAX_CODECS) {
                printk(KERN_ERR "zr36050: Can't attach more codecs!\n");
                return -ECANCELED;
        }
        ...
        if (NULL == ptr) {
                printk(KERN_ERR "zr36050: Can't get enough memory!\n");
                return -ENOMEM;
        }
