...
//#define MODVERSIONS // inhibit symbol inclusion
...
struct videocodec *videocodec_attach(struct videocodec_master *master)
{
        ...
        if (!master) {
                printk(KERN_ERR "videocodec_attach: no data\n");
                return NULL;
        }

        DEBUG1("videocodec_attach: '%s', type: %x, flags %lx, magic %lx\n",
               master->name, master->type, master->flags, master->magic);

        if (!h) {
                printk(KERN_ERR "videocodec_attach: no device available\n");
                return NULL;
        }
        ...
        while (h) {
                ...
                if ((master->flags & h->codec->flags)==master->flags) {
                        DEBUG3("videocodec_attach: try '%s'\n",
                               h->codec->name);
                        codec = kmalloc(sizeof(struct videocodec),GFP_KERNEL);
                        if (!codec) {
                                printk(KERN_ERR "videocodec_attach: no mem\n");
                                return NULL;
                        }
                        ...
                        if (res==0) {
                                DEBUG2("videocodec_attach '%s'\n",
                                       codec->name);
                                ptr = (struct attached_list *)kmalloc(
                                ...
                                memset(ptr,0,sizeof(struct attached_list));
                                ptr->codec = codec;

                                a=h->list;
                                if (!a) {
                                        h->list = ptr;
                                        DEBUG3("videocodec: first element\n");
                                } else {
                                        while (a->next) a=a->next; // find end
                                        a->next = ptr;
                                        DEBUG3("videocodec: in after '%s'\n",
                                               h->codec->name);
                                }
        ...
        printk(KERN_ERR "videocodec_attach: no codec found!\n");
        return NULL;
}
int videocodec_detach(struct videocodec *codec)
{
        ...
        if (!codec) {
                printk(KERN_ERR "videocodec_detach: no data\n");
                return -EINVAL;
        }

        DEBUG1("videocodec_detach: '%s', type: %x, flags %lx, magic %lx\n",
               codec->name, codec->type, codec->flags, codec->magic);

        if (!h) {
                printk(KERN_ERR "videocodec_detach: no device left...\n");
                return -ENXIO;
        }
        ...
        while (h) {
                ...
                while (a) {
                        if (codec==a->codec) {
                                ...
                                if (res>=0) {
                                        DEBUG2("videocodec_detach: '%s'\n",
                                               a->codec->name);
                                        a->codec->master_data = NULL;
                                } else {
                                        printk(KERN_ERR "videocodec_detach: '%s'\n",
                                               a->codec->name);
                                        a->codec->master_data = NULL;
                                }
                                if (prev==NULL) {
                                        h->list = a->next;
                                        DEBUG3("videocodec: delete first\n");
                                } else {
                                        prev->next = a->next;
                                        DEBUG3("videocodec: delete middle\n");
                                }
                                kfree(a->codec);
                                kfree(a);
                                h->attached -= 1;
        ...
        printk(KERN_ERR "videocodec_detach: given codec not found!\n");
        return -EINVAL;
}
...
int videocodec_register(const struct videocodec *codec)
{
        ...
        if (!codec) {
                printk(KERN_ERR "videocodec_register: no data!\n");
                return -EINVAL;
        }

        DEBUG1("videocodec: register '%s', type: %x, flags %lx, magic %lx\n",
               codec->name, codec->type, codec->flags, codec->magic);

        ptr = (struct codec_list *)kmalloc(sizeof(struct codec_list),
                                           GFP_KERNEL);
        if (!ptr) {
                printk(KERN_ERR "videocodec_register: no memory\n");
                return -ENOMEM;
        }
        ...
        if (!h) {
                codeclist_top = ptr;
                DEBUG3("videocodec: hooked in as first element\n");
        } else {
                while (h->next) h = h->next;  // find the end
                h->next = ptr;
                DEBUG3("videocodec: hooked in after '%s'\n", h->codec->name);
        }
        MOD_INC_USE_COUNT;
        return 0;
}
...
int videocodec_unregister(const struct videocodec *codec)
{
        ...
        if (!codec) {
                printk(KERN_ERR "videocodec_unregister: no data!\n");
                return -EINVAL;
        }

        DEBUG1("videocodec: unregister '%s', type: %x, flags %lx, magic %lx\n",
               codec->name, codec->type, codec->flags, codec->magic);

        if (!h) {
                printk(KERN_ERR "videocodec_unregister: no device left...\n");
                return -ENXIO;
        }
        ...
        while (h) {
                if (codec==h->codec) {
                        if (h->attached) {
                                printk(KERN_ERR "videocodec: '%s' is used\n",
                                                  h->codec->name);
                                return -EBUSY;
                        }
                        DEBUG2("videocodec: unregister '%s' is ok.\n",h->codec->name);
                        if (prev==NULL) {
                                codeclist_top = h->next;
                                DEBUG3("videocodec: delete first element\n");
                        } else {
                                prev->next = h->next;
                                DEBUG3("videocodec: delete middle element\n");
                        }
                        kfree(h);
                        MOD_DEC_USE_COUNT;
                        return 0;
        ...
        printk(KERN_ERR "videocodec_unregister: given codec not found!\n");
        return -EINVAL;
}
...
static int videocodec_build_table(void)
{
        ...
        if (videocodec_buf) kfree(videocodec_buf);
...
static int videocodec_info(char *buffer,char **buffer_location,
                    off_t offset,int buffer_length,int *eof, void *data)
{
        int size;

        DEBUG2("videocodec_info: offset: %ld, len %d / size %d\n",
        ...
        if ((offset<0) || (offset>=videocodec_bufsize)) {
                DEBUG3("videocodec_info: call delivers no result, return 0\n");
                *eof=1;
                return 0;
        }

        if (buffer_length<(videocodec_bufsize-offset)) {
                DEBUG3("videocodec_info: %ld needed, %d got\n",
                       videocodec_bufsize-offset, buffer_length);
                size=buffer_length;
        } else {
                DEBUG3("videocodec_info: last reading of %ld bytes\n",
...
int __init videocodec_init(void)
{
        ...
        if (videocodec_proc_entry) {
                ...
                printk(KERN_ERR "videocodec: can't init procfs.\n");
        }
#endif
        return 0;
}
