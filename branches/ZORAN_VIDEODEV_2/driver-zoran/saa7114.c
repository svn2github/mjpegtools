...
static inline int
saa7114_write(struct i2c_client *client, u8 reg, u8 value)
{
...
static int
saa7114_write_block(struct i2c_client *client, const u8 *data, unsigned int len)
{
   int ret=-1;
   u8 reg;
   ...
   if (i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
      ...
      while (len >= 2) {
         ...
         do {
            block_data[msg.len++] = decoder->reg[reg++] = data[1];
            len -= 2; data += 2;
      ...
      while (len >= 2) {
         reg = *data++;
         if ((ret = saa7114_write(client, reg, *data++)) < 0)
            break;
         len-=2;
      }
   }
...
static inline int
saa7114_read(struct i2c_client *client, u8 reg)
{
...
static const unsigned char init[] = {
   0x00, 0x00,      /* 00 - ID byte , chip version, read only */
   0x01, 0x08,      /* 01 - X,X,X,X, IDEL3 to IDEL0 - horizontal increment delay, recommended position */
   0x02, 0x00,      /* 02 - FUSE=3, GUDL=2, MODE=0 ; input control */
   0x03, 0x10,      /* 03 - HLNRS=0, VBSL=1, WPOFF=0, HOLDG=0, GAFIX=0, GAI1=256, GAI2=256 */
   0x04, 0x90,      /* 04 - GAI1=256 */
   0x05, 0x90,      /* 05 - GAI2=256 */
   0x06, SAA_7114_NTSC_HSYNC_START,      /* 06 - HSB: hsync start, depends on the video standard */
   0x07, SAA_7114_NTSC_HSYNC_STOP,      /* 07 - HSS: hsync stop, depends on the video standard */
   0x08, 0xb8,      /* 08 - AUFD=1, FSEL=1, EXFIL=0, VTRC=1, HPLL: free running in playback, locked in capture, VNOI=0 */
   0x09, 0x80,      /* 09 - BYPS=0, PREF=0, BPSS=0, VBLB=0, UPTCV=0, APER=1; depends from input */
   0x0a, 0x80,      /* 0a - BRIG=128 */
   0x0b, 0x44,      /* 0b - CONT=1.109 */
   0x0c, 0x40,      /* 0c - SATN=1.0 */
   0x0d, 0x00,      /* 0d - HUE=0 */
   0x0e, 0x84,      /* 0e - CDTO, CSTD2 to 0, DCVF, FCTC, CCOMB; depends from video standard */
   0x0f, 0x24,      /* 0f - ACGC,CGAIN6 to CGAIN0; depends from video standard */
   0x10, 0x03,      /* 10 - OFFU1 to 0, OFFV1 to 0, CHBW, LCBW2 to 0 */
   0x11, 0x59,      /* 11 - COLO, RTP1, HEDL1 to 0, RTP0, YDEL2 to 0 */
   0x12, 0xc9,      /* 12 - RT signal control RTSE13 to 10 and 03 to 00 */
   ...
   0xa1, 0x00,      // horizontal prescaler accumulation sequence length
   0xa2, 0x00,      // UV FIR filter, Y FIR filter, prescaler DC gain
   0xa3, 0x00,
...
static int saa7114_command(struct i2c_client *client, unsigned int cmd, void * arg)
{
   struct saa7114 *decoder = client->data;
   ...
   case 0:
//        printk(KERN_INFO "%s: writing init\n", client->name);
//        saa7114_write_block(client, init, sizeof(init));
   ...
   case DECODER_DUMP:
	 ...
	 printk(KERN_INFO "%s: decoder dump\n", client->name);
         ...
         for (i = 0; i < 32; i += 16) {
            ...
            for (j = 0; j < 16; ++j) {
               printk(" %02x", saa7114_read(client, i + j));
            }
            printk("\n");
         }
      }
   ...
   case DECODER_GET_CAPABILITIES:
      {
         ...
         DEBUG(printk(KERN_DEBUG "%s: decoder get capabilities\n", client->name));
   ...
   case DECODER_SET_NORM:
	 ...
	 DEBUG(printk(KERN_DEBUG "%s: decoder set norm ", client->name));
         switch (*iarg) {

         case VIDEO_MODE_NTSC:
            DEBUG(printk("NTSC\n"));
            decoder->reg[REG_ADDR(0x06)] = SAA_7114_NTSC_HSYNC_START;
            decoder->reg[REG_ADDR(0x07)] = SAA_7114_NTSC_HSYNC_STOP;
         ...
         case VIDEO_MODE_PAL:
            DEBUG(printk("PAL\n"));
            decoder->reg[REG_ADDR(0x06)] = SAA_7114_PAL_HSYNC_START;
            decoder->reg[REG_ADDR(0x07)] = SAA_7114_PAL_HSYNC_STOP;
         ...
         case VIDEO_MODE_SECAM:
            DEBUG(printk("SECAM\n"));
//             saa7114_write(client, 0x08, (decoder->reg[0x0e] & 0x3f) |0x00);
//             saa7114_write(client, 0x0e, (decoder->reg[0x0e] & 0x8f) |0x50);
//            break;
            return -EINVAL;
 
         case VIDEO_MODE_AUTO:
            DEBUG(printk("AUTO\n"));
//            saa7114_write(client, 0x08, (decoder->reg[0x08] & 0x3f) | 0x80);
//            saa7114_write(client, 0x0e, (decoder->reg[0x0e] & 0x8f));
//            break;
            return -EINVAL;
         
         default:
            DEBUG(printk(" Unknown video mode!!!\n"));
            return -EINVAL;
         ...
         saa7114_write_block(client, decoder->reg + (0x06 << 1), 3 << 1);
         saa7114_write_block(client, decoder->reg + (0x0e << 1), 2 << 1);
         saa7114_write_block(client, decoder->reg + (0x5a << 1), 2 << 1);

         saa7114_write_block(client, decoder->reg + (0x94 << 1), (0x9f + 1 - 0x94) << 1);
         saa7114_write_block(client, decoder->reg + (0xc4 << 1), (0xcf + 1 - 0xc4) << 1);
   ...
   case DECODER_SET_INPUT:
      {
         ...
         DEBUG(printk(KERN_DEBUG "%s: decoder set input (%d)\n", client->name, *iarg));         
         if (*iarg < 0 || *iarg > 7) {
            return -EINVAL;
         }

         if (decoder->input != *iarg) {
            DEBUG(printk(KERN_DEBUG "%s: now setting %s input\n", client->name, *iarg >= 6 ? "S-Video" : "Composite"));
            decoder->input = *iarg;
            
            /* select mode */
            decoder->reg[REG_ADDR(0x02)] = (decoder->reg[REG_ADDR(0x02)] & 0xf0) | (decoder->input < 6 ? 0x0 : 0x9);
            saa7114_write(client, 0x02, decoder->reg[REG_ADDR(0x02)] );

            /* bypass chrominance trap for modes 6..9 */
            decoder->reg[REG_ADDR(0x09)] = (decoder->reg[REG_ADDR(0x09)] & 0x7f) | (decoder->input < 6  ? 0x0: 0x80 );
            saa7114_write(client, 0x09, decoder->reg[REG_ADDR(0x09)] );

            decoder->reg[REG_ADDR(0x0e)] = decoder->input < 6 ? decoder->reg[REG_ADDR(0x0e)] | 1 : decoder->reg[REG_ADDR(0x0e)] & ~1;
   ...
   case DECODER_SET_OUTPUT:
      {
         ...
         DEBUG(printk(KERN_DEBUG "%s: decoder set output\n", client->name)); 
   ...
   case DECODER_ENABLE_OUTPUT:
      {
         ...
         DEBUG(printk(KERN_DEBUG "%s: decoder %s output\n", client->name, enable ? "enable" : "disable"));
...
//     If output should be enabled, we have to reverse the above.
            ...
            saa7114_write_block(client, decoder->reg + (0x12 << 1), 2 << 1);
            saa7114_write(client, 0x08, decoder->reg[REG_ADDR(0x08)]);
            saa7114_write(client, 0x87, decoder->reg[REG_ADDR(0x87)]);
            saa7114_write(client, 0x88, 0xd8); // sw reset scaler
            saa7114_write(client, 0x88, 0xf8); // sw reset scaler release            
   ...
   case DECODER_SET_PICTURE:
      {
         ...
         DEBUG(printk(KERN_DEBUG "%s: decoder set picture bright=%d contrast=%d saturation=%d hue=%d\n", 
		client->name, pic->brightness, pic->contrast, pic->colour, pic->hue));
         ...
         if (decoder->contrast != pic->contrast) {
            ...
            saa7114_write(client, 0x0b, decoder->contrast >> 9);
         }
         ...
         if (decoder->hue != pic->hue) {
            ...
            decoder->hue = pic->hue;
            saa7114_write(client, 0x0d, (decoder->hue - 32768) >> 8);
         }
      }
      break;
 ...
 */
static unsigned short normal_i2c[] = { I2C_SAA7114>>1, I2C_SAA7114A>>1, I2C_CLIENT_END };
                       ...
                       int                 kind)
{
   ...
   DEBUG(printk(KERN_INFO "saa7114.c: detecting saa7114 client on address 0x%x\n", address<<1));
   ...
   snprintf(client->name, sizeof(client->name)-1, "saa7114[%d]", client->id);

   client->data = decoder = kmalloc(sizeof(struct saa7114), GFP_KERNEL);
   if (decoder == NULL) {
      kfree(client);
      return -ENOMEM;
   }
   ...
   decoder->reg[REG_ADDR(0xb8)] = LOBYTE(LOWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xb9)] = HIBYTE(LOWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xba)] = LOBYTE(HIWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xbb)] = HIBYTE(HIWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));

   decoder->reg[REG_ADDR(0xbc)] = LOBYTE(LOWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xbd)] = HIBYTE(LOWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xbe)] = LOBYTE(HIWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xbf)] = HIBYTE(HIWORD(SAA_7114_VERTICAL_LUMA_OFFSET));

   decoder->reg[REG_ADDR(0xe8)] = LOBYTE(LOWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xe9)] = HIBYTE(LOWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xea)] = LOBYTE(HIWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));
   decoder->reg[REG_ADDR(0xeb)] = HIBYTE(HIWORD(SAA_7114_VERTICAL_CHROMA_OFFSET));

   decoder->reg[REG_ADDR(0xec)] = LOBYTE(LOWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xed)] = HIBYTE(LOWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xee)] = LOBYTE(HIWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   decoder->reg[REG_ADDR(0xef)] = HIBYTE(HIWORD(SAA_7114_VERTICAL_LUMA_OFFSET));
   ...
   DEBUG(printk(KERN_DEBUG "%s_attach: starting decoder init\n", client->name));

   err[0] = saa7114_write_block(client, decoder->reg + (0x20 << 1), 0x10 << 1);
   err[1] = saa7114_write_block(client, decoder->reg + (0x30 << 1), 0x10 << 1);
   err[2] = saa7114_write_block(client, decoder->reg + (0x63 << 1), (0x7f + 1 - 0x63) << 1);
   err[3] = saa7114_write_block(client, decoder->reg + (0x89 << 1), 6 << 1);
   err[4] = saa7114_write_block(client, decoder->reg + (0xb8 << 1), 8 << 1);
   err[5] = saa7114_write_block(client, decoder->reg + (0xe8 << 1), 8 << 1);
   ...
   { 
     if (err[i] < 0)
     {
       printk(KERN_ERR "%s_attach: init error %d at stage %d, leaving attach.\n",client->name, i, err[i]); 
       return 0;
     }
   }
   ...
   for(i = 6; i < 8; i++)
   {
     DEBUG(printk(KERN_DEBUG "%s_attach: reg[0x%02x] = 0x%02x (0x%02x)\n",
       client->name, i, saa7114_read(client, i), decoder->reg[REG_ADDR(i)]));
   }

   DEBUG(printk(KERN_DEBUG "%s_attach: performing decoder reset sequence\n", client->name));
   ...
   DEBUG(printk(KERN_INFO "%s_attach: performing the rest of init\n", client->name));
   ...
   err[15] = saa7114_write_block(client, decoder->reg + (0x94 << 1), 12 << 1); 
   err[16] = saa7114_write_block(client, decoder->reg + (0xa0 << 1), 8 << 1); 
   err[17] = saa7114_write_block(client, decoder->reg + (0xa8 << 1), 8 << 1); 
   err[18] = saa7114_write_block(client, decoder->reg + (0xb0 << 1), 8 << 1); 
   err[19] = saa7114_write_block(client, decoder->reg + (0xc0 << 1), 4 << 1); // Task B
   err[15] = saa7114_write_block(client, decoder->reg + (0xc4 << 1), 12 << 1);
   err[16] = saa7114_write_block(client, decoder->reg + (0xd0 << 1), 8 << 1); 
   err[17] = saa7114_write_block(client, decoder->reg + (0xd8 << 1), 8 << 1); 
   err[18] = saa7114_write_block(client, decoder->reg + (0xe0 << 1), 8 << 1); 
   ...
   DEBUG(printk(KERN_DEBUG "%s_attach: setting video input\n", client->name));

   err[19] = saa7114_write(client, 0x02, decoder->reg[REG_ADDR(0x02)]);
   err[20] = saa7114_write(client, 0x09, decoder->reg[REG_ADDR(0x09)]); 
   err[21] = saa7114_write(client, 0x0e, decoder->reg[REG_ADDR(0x0e)]); 
   ...
   printk(KERN_INFO "%s_attach: chip version %x, decoder status 0x%02x\n", 
           client->name, saa7114_read(client, 0x00)>>4, saa7114_read(client, 0x1f));
   DEBUG(printk(KERN_DEBUG "%s_attach: power save control: 0x%02x, scaler status: 0x%02x\n",
           client->name, saa7114_read(client, 0x88), saa7114_read(client,0x8f)));
   ...
   if (i < 0) {
      printk(KERN_ERR "%s_attach error: init status %d\n", client->name, i);
   } else {
      printk(KERN_INFO "%s_attach: chip version %x at address 0x%x\n",
         client->name, saa7114_read(client, 0x00)>>4, client->addr<<1);
   }
...
{
   DEBUG(printk(KERN_INFO "saa7114.c: starting probe for adapter %s (0x%x)\n",
