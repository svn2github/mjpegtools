...
static __u8 zr36016_read(struct zr36016 *ptr, __u16 reg)
{       __u8 value = 0;
...
static void zr36016_write(struct zr36016 *ptr, __u16 reg, __u8 value)
{
...
{       __u8 value = 0;
        ...
        if ( ( ptr->codec->master_data->writereg ) &&
             ( ptr->codec->master_data->readreg ) ) {
                ...
                printk(KERN_ERR "%s: invalid I/O setup, nothing read (i)!\n", ptr->name);

        DEBUG3("%s: reading indirect from 0x%04x: %02x\n",ptr->name,reg,value);
        return value;
}

static void zr36016_writei(struct zr36016 *ptr, __u16 reg, __u8 value)
{
        DEBUG3("%s: writing indirect 0x%02x to 0x%04x\n",ptr->name,value,reg);
        ...
        if ( ptr->codec->master_data->writereg ) {
                ...
                printk(KERN_ERR "%s: invalid I/O setup, nothing written (i)!\n", ptr->name);
}
...
static __u8 zr36016_read_version(struct zr36016 *ptr)
{
...
static int zr36016_basic_test(struct zr36016 *ptr)
{
		...
		int i;
                ...
                printk(KERN_INFO "%s: registers: ", ptr->name);
                for (i=0;i<=0x0b;i++) printk("%02x ",zr36016_readi(ptr,i));
                printk("\n");
        }
        ...
        if ( zr36016_readi(ptr,ZR016I_PAX_LO) != 0x0 ) {
                printk(KERN_ERR "%s: attach failed, can't connect to vfe processor!\n",
                       ptr->name);
                return -ENXIO;
        }
        ...
        if ( ptr->version & 0x0c ) {
                printk(KERN_ERR "%s: attach failed, suspicious version %d found...\n",
...
/* static int zr36016_pushit(struct zr36016 *ptr, __u16 startreg, __u16 len,
                          const char data[])
{
        int i=0;
...
{       
        ...
        zr36016_write(ptr, ZR016_MODE, ZR016_YUV422 | ZR016_YUV422_YUV422 |
                           (ptr->mode==CODEC_DO_COMPRESSION?ZR016_COMPRESSION:
                                                            ZR016_EXPANSION));
	// Window setup
...
static void zr36016_inc_use(void)
{
        MOD_INC_USE_COUNT;
}

static void zr36016_dec_use(void)
{
        MOD_DEC_USE_COUNT;
}
...
int zr36016_unset(struct videocodec *codec)
...
int zr36016_setup(struct videocodec *codec)
{
        struct zr36016 *ptr;
        int res;

        DEBUG1("zr36016: initializing VFE subsystem #%d.\n",
               zr36016_codecs);
        ...
        if (NULL == ptr) {
                printk(KERN_ERR "zr36016: Can't get enough memory!\n");
                return -ENOMEM;
        }
        ...
        printk(KERN_INFO "%s: codec v%d attached and running\n",ptr->name,ptr->version);
